...
def detect_supply_demand_zones(candles, current_price=None, lookback=3, max_zones=2):
    ...
    for i in range(lookback, len(candles) - lookback - 4):
        window = candles[i - lookback:i + lookback + 1]
        center = candles[i]
        ...
        if center["low"] == min(c["low"] for c in window):
            ...
            zone_low = center["low"]
            zone_high = min(body_high, center["low"] + max(body_size, candle_range * 0.45))

            if zone_width_ok(zone_low, zone_high, current_price=current_price, max_bps=45):
                zone = build_zone(
                    "demand",
                    zone_low,
                    zone_high,
                ...
                if zone and zone["quality_score"] >= 55:
                    demand.append(zone)
        ...
        if center["high"] == max(c["high"] for c in window):
            ...
            zone_low = max(body_low, center["high"] - max(body_size, candle_range * 0.45))
