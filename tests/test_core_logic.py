...
class CoreLogicTests(unittest.TestCase):
    ...
    def test_previous_weekday_skips_weekend(self):
        self.assertEqual(server_stream.previous_weekday(date(2026, 6, 15)), date(2026, 6, 12))
        self.assertEqual(server_stream.previous_weekday(date(2026, 6, 16)), date(2026, 6, 15))

    def test_market_session_status_is_deterministic(self):
        weekend = server_stream.build_market_session_status(datetime(2026, 6, 14, 10, tzinfo=ET))
        self.assertEqual(weekend["session_label"], "CLOSED")
        self.assertEqual(weekend["market_closed_reason"], "Weekend")
        regular = server_stream.build_market_session_status(datetime(2026, 6, 15, 10, tzinfo=ET))
        self.assertEqual(regular["session_label"], "REGULAR")
        self.assertTrue(regular["is_regular_session_open"])
        premarket = server_stream.build_market_session_status(datetime(2026, 6, 15, 8, tzinfo=ET))
    ...
    def test_ema_and_vwap_use_candle_values(self):
        ...
        self.assertEqual([point["value"] for point in server_stream.calc_ema(candles, 3)], [10, 11.0, 12.5])
        self.assertEqual([point["value"] for point in server_stream.calc_vwap(candles)], [10.0, 11.0, 12.0])

    def test_cluster_levels_and_simple_support_resistance(self):
        clusters = server_stream.cluster_levels([100.00, 100.04, 101.00])
    ...
    def test_fallback_review_stays_read_only_without_setup(self):
        review = server_stream.build_fallback_ai_review({
            ...
            "market_context": {},
            "timeframes": {},
            "market_session_status": server_stream.build_market_session_status(datetime(2026, 6, 15, 10, tzinfo=ET)),
        })
