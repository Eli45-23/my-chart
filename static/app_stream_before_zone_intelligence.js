...
function drawStaticLevels(data) {
  ...
  if (layerState.sr) {
    ...
    (sr.resistance || []).forEach((level, index) => {
      addLevel(`R${index + 1}`, level.price, COLORS.resistance, LightweightCharts.LineStyle.Dashed);
    });

    (sr.support || []).forEach((level, index) => {
      addLevel(`S${index + 1}`, level.price, COLORS.support, LightweightCharts.LineStyle.Dashed);
    });
  }
