...
function addZoneBand(label, low, high, color) {
  if (low === null || low === undefined || high === null || high === undefined) return;

  addLevel(`${label} High`, high, color, LightweightCharts.LineStyle.Dotted);
  addLevel(`${label} Low`, low, color, LightweightCharts.LineStyle.Dotted);
}
...
function updateLegend(data) {
  ...
  legendEl.innerHTML = [
    ...
    textPill(`Demand: ${(latestPayload.supply_demand?.demand || []).map(z => `${z.low.toFixed(2)}-${z.high.toFixed(2)}`).join(", ") || "none"}`),
    textPill(`Supply: ${(latestPayload.supply_demand?.supply || []).map(z => `${z.low.toFixed(2)}-${z.high.toFixed(2)}`).join(", ") || "none"}`),
...
function drawStaticLevels(data) {
  ...
  if (layerState.supplyDemand) {
    ...
    (sd.supply || []).forEach((zone, index) => {
      addZoneBand(`Supply ${index + 1}`, zone.low, zone.high, "#e57373");
    });

    (sd.demand || []).forEach((zone, index) => {
      addZoneBand(`Demand ${index + 1}`, zone.low, zone.high, "#81c784");
    });
  }
